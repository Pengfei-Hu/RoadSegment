...
#   capture_id = get_uuid()
    pic_name = 'map/{}/{}_{}_{}_{}.png'.format(source, lat, lon, tileZoom, "original")
...
def multi_pic_whole(lat, lon, tileZoom, multi, source):
    ...
    for ty in range(start_ty, end_ty + 1):
        for tx in range(start_tx, end_tx + 1):
            ...
            pic_name = 'map/{}/combine/single/{}_{}_{}_{}.png'.format(source, lat, lon, tileZoom + multi, loc)
            # Save Images
            with open(pic_name, 'wb') as out_file:
                out_file.write(response.content)
            loc += 1
    ...
    for y in range(1, IMAGE_ROW + 1):
        for x in range(1, IMAGE_COLUMN + 1):
            from_image = Image.open('map/{}/combine/single/{}_{}_{}_{}.png'.format(source, lat, lon, tileZoom + multi, i))
            to_image.paste(from_image, ((x - 1) * IMAGE_SIZE, (y - 1) * IMAGE_SIZE))
            i += 1
    pic_url = 'map/{}/combine/{}_{}_{}_{}.png'.format(source, lat, lon, tileZoom + multi, "whole")
...
def multi_all():
    ...
    bing_url = 'map/{}/combine/single/{}_{}_{}_'.format('bing', lat, lon, tileZoom + endZoomLevel)
    location_photos.inser_map_all(lat, lon, 'B', tileZoom, bing_url,endZoomLevel,dic)
    bing_whole = download_tile(lat, lon, tileZoom,'bing')
    bing_name = 'map/{}/combine/{}_{}_{}_{}.png'.format('bing', lat, lon, tileZoom + endZoomLevel, "whole")
    location_photos.inser_map_whole(lat, lon, 'B', tileZoom, bing_name, endZoomLevel)
    #bing_original = 'map/{}/{}_{}_{}_{}.png'.format('bing', lat, lon, tileZoom, "oringinal")
    ...
    google_url = 'map/{}/combine/single/{}_{}_{}_'.format('google', lat, lon, tileZoom + endZoomLevel)
    ...
    google_name = 'map/{}/combine/{}_{}_{}_{}.png'.format('google', lat, lon, tileZoom + endZoomLevel, "whole")
    location_photos.inser_map_whole(lat, lon, 'G', tileZoom, google_name, endZoomLevel)
    #google_original = 'map/{}/{}_{}_{}_{}.png'.format('google', lat, lon, tileZoom, "original")
    ...
    osm_url = 'map/{}/combine/single/{}_{}_{}_'.format('osm', lat, lon, tileZoom + endZoomLevel)
    location_photos.inser_map_all(lat, lon, 'O', tileZoom, osm_url,endZoomLevel,dic)
    osm_whole = download_tile(lat, lon, tileZoom,'osm')
    osm_name = 'map/{}/combine/{}_{}_{}_{}.png'.format('osm', lat, lon, tileZoom + endZoomLevel, "whole")
    location_photos.inser_map_whole(lat, lon, 'O', tileZoom, osm_name, endZoomLevel)
    osm_original = 'map/{}/{}_{}_{}_{}.png'.format('osm', lat, lon, tileZoom, "original")
