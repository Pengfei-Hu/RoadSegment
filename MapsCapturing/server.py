...
def multi_pic_whole(lat, lon, tileZoom, multi, source):
    ...
    for ty in range(start_ty, end_ty + 1):
        for tx in range(start_tx, end_tx + 1):
            ...
            if source == 'google':
                url = 'http://mt0.google.com/vt/lyrs=m@174000000&src=app&x={}&s=&y={}&z={}'.format(tx, ty,
                                                                                                   tileZoom + multi)
            elif source == 'bing':
                new_qkStr = t.TileXYToQuadKey(tx, ty, tileZoom + multi)
...
def multi_pic_part(lat, lon, tileZoom, multi, source, part_list):
    ...
    if source == 'google':
        url = 'http://mt0.google.com/vt/lyrs=m@174000000&src=app&x={}&s=&y={}&z={}'.format(new_tx, new_ty,
                                                                                           tileZoom + multi)
    elif source == 'bing':
...
def multi_all():
    ...
    google_url = 'map/{}/combine/single/'.format('google')
